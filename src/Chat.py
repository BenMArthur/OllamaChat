...
class Chat(QMainWindow):
    ...
    def __init__(self, screen, appName):
        ...
        self.ui.newButton.clicked.connect(lambda: self.newChat())
        ...
        self.ui.userClosed.connect(self.settings.hide)
    ...
    def addHiddenPromptIfNeeded(self, text):
        try:
            if text:
                ...
                if len(split)>0:
                    if not split[0].startswith(self.delims["system"]) and self.sysPrompt.strip() != "" and self.enableSysPrompt:
        ...
        try:
            ...
            if (self.dataStore / f"history/temp{os.getpid()}/{self.ui.historyNames[self.ui.historySelect.currentIndex()]}.txt").is_file() and not loadPerm:
                with open(self.dataStore / f"history/temp{os.getpid()}/{self.ui.historyNames[self.ui.historySelect.currentIndex()]}.txt", "r", encoding="utf-8") as file:
                    ...
                    split = self.splitText(text)
                    if len(split) >= 2:
                        if split[0].startswith(self.delims["system"]) and self.sysPrompt.strip() != "":
